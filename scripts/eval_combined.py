...
class ControllerDDPF:
    def __init__(self, cfg, cc_params, env):
        ...
        self.current_particles = None  # Store current particles for visualization
        self.selected_particle_idx = None  # Store selected particle indices
 ...
 # 1. Retrieve Raw Maps
            ...
            self.current_particles = x.view(N, self.num_particles, self.pred_horizon, 2)  # (N, K, 16, 2)
            self.selected_particle_idx = best_idx
            # Gather best
    ...
    def get_particles_for_viz(self, agent_positions):
        """
        Returns particle trajectories in world coordinates for visualization
        """
        if self.current_particles is None:
            return None
        
        N = self.current_particles.shape[0]
        particles_world = []
        
        for agent_idx in range(N):
            agent_pos = agent_positions[agent_idx]
            particles = self.current_particles[agent_idx].cpu().numpy()  # (K, 16, 2)
            
            # Convert relative actions to absolute world positions
            trajectories = []
            for k in range(self.num_particles):
                traj = np.cumsum(particles[k], axis=0) + agent_pos  # Accumulate actions from current position
                trajectories.append(traj)
            
            particles_world.append({
                'trajectories': np.array(trajectories),  # (K, 16, 2)
                'selected_idx': self.selected_particle_idx[agent_idx].item()
            })
...
class EvaluatorSingle:
    def __init__(self, in_config):
        ...
        if "FeatureFile" in self.config and "RobotPosFile" in self.config:
            ...
            self.env_main = CoverageSystem(self.cc_params)
            self.world_idf = self.env_main.GetWorldIDF()
    def render_particle_video(self, env, title, history_pos, history_particles, filename):
        fig, ax = plt.subplots(figsize=(8, 8))
    
    # 1. Setup Static Map (Obstacles)
    # Get obstacle map from env (assuming 32x32 grid)
        obst_map = CoverageEnvUtils.get_raw_obstacle_maps(env, self.cc_params)
    # If obst_map is tensor, convert to numpy. 
    # Usually it returns (N, 1, 32, 32) or similar.
        if hasattr(obst_map, 'cpu'): obst_map = obst_map.cpu().numpy()
    
    # Just use the first agent's map for background if they are all same
        if obst_map.ndim == 4: grid = obst_map[0, 0] 
        elif obst_map.ndim == 3: grid = obst_map[0]
        else: grid = obst_map
        ...
        # Draw Obstacles (Black)
            ax.imshow(grid, cmap='Greys', origin='lower', extent=[-2, 32+2, -2, 32+2], alpha=0.3)
            ax.set_xlim(0, 32)
            ax.set_ylim(0, 32)
        
        # Get data for this frame
            current_pos = history_pos[frame_idx]     # List of [x, y]
            particles = history_particles[frame_idx] # (N, K, Hor, 2) or None

        # Draw Agents
            colors = plt.cm.rainbow(np.linspace(0, 1, len(current_pos)))
            for i, pos in enumerate(current_pos):
            # Draw Robot
                ax.plot(pos[0], pos[1], 'o', color=colors[i], markersize=10, markeredgecolor='k')
            
            # Draw Particles
                if particles is not None:
                # particles[i] is (K, Horizon, 2) - these are actions (velocities)
                # We need to compute absolute positions for visualization
                # Trajectory = Cumulative Sum of Actions + Start Pos
                
                    agent_particles = particles[i] # (K, H, 2)
                    K = agent_particles.shape[0]
                
                    for k in range(0, K, 2): # Skip every 2nd particle to reduce clutter if needed
                    # Calculate trajectory points
                    # actions are typically dx, dy. 
                        traj_actions = agent_particles[k]
                        traj_points = np.zeros((traj_actions.shape[0] + 1, 2))
                        traj_points[0] = pos
                        traj_points[1:] = pos + np.cumsum(traj_actions, axis=0)
                    
                    # Plot thin line
                        ax.plot(traj_points[:, 0], traj_points[:, 1], 
                                color=colors[i], alpha=0.1, linewidth=0.5)

        ani = animation.FuncAnimation(fig, update, frames=len(history_pos), interval=100)
    ...
    def evaluate(self, save=True):
        ...
        # ... existing cost recording ...

        # --- CAPTURE HISTORY ---
                if self.generate_video:
            # 1. Get Robot Positions
                    pos = env.GetRobotPositions()
            # Convert to list of [x, y]
                    current_pos = []
                    for p in pos:
                        if hasattr(p, 'x'):
                            current_pos.append([p.x, p.y])
                        elif isinstance(p, (list, tuple, np.ndarray)):
                            current_pos.append([p[0], p[1]])
                        else:
        # Fallback if it's something else, or print to debug
                            current_pos.append([0.0, 0.0])

                    history_pos.append(current_pos)
            
            # 2. Get Particles from Controller
                    if hasattr(controller, 'viz_particles') and controller.viz_particles is not None:
                # Shape: (N_agents, N_particles, Horizon, 2)
                        history_particles.append(controller.viz_particles) 
                    else:
                        history_particles.append(None) # You may need to modify this to accept particles_data
            ...
            if save:
                ...
                if self.generate_video:
                    try:
                        print("Rendering custom video with particles...")
                        self.render_particle_video(
                            env,
                            controller_name,
                            history_pos,
                            history_particles,
                            os.path.join(c_dir, "video_with_particles.mp4")
                        )
...
# Add inside EvaluatorSingle class
